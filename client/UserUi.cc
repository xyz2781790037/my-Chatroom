    ...
    while(1){
        if(Type::getUserState() == Type::URETURN){
            ...
            if(cmd == "/quit" || cmd == "\u001b"){
                ...
                std::cout << "/af username yes/no" << std::endl;
                std::cout << "/ag user:group yes/no" << std::endl;
                std::cout << "/iv groupname yes/no" << std::endl;
                continue;
            }
            else{
                tool::segExcessiveSpace(cmd);
            }
            if(handleCmd(cmd)){
                Type::updataUserState(Type::UWAIT);
            }
...
void Userui::fileSystem(std::string name){
    ...
    while (true)
    {
        ...
        if(input == tool::tolowerStr("LIST")){
        ...
        else if(input.find_first_of(' ') != std::string::npos && input.find_first_of(' ') == input.find_last_of(' ')){
            if(input.substr(0,4) == tool::tolowerStr("STOR")){
                ...
                if (ftpConn && ftpConn->connected()) {
                    ...
                    ftpConn->send(MessageSplitter::encodeMessage("info " + user_->getUserName() + name + " " + user_->getUserMyname() + " " + input.substr(5) + " " + tool::fileSize(fd)));
                    tool::clearInputLines(input);
                } else {
                    LOG_ERROR << "conn过期";
                }
