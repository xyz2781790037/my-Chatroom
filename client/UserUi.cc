...
Userui::Userui(std::shared_ptr<User> user,
               const mulib::net::TcpClient::TcpConnectionPtr& Conn,
               const mulib::net::TcpClient::TcpConnectionPtr ftpConn,
               mulib::base::Timestamp recviveTime)
    : user_(user), conn(Conn), recviveTime_(recviveTime), ftpConn(ftpConn) {}
void Userui::ui() {
    Presence = true;
    online("online");
    uiStart = false;
    while (Presence) {
        ...
        if (Type::getUserState() == Type::UEXECUTE) {
            updataMessage();
            std::cout << COLOUR1 << "     你好, " << user_->getUserName()
                      << COLOUREND << std::endl;
...
void Userui::selectFunc(std::string select) {
    ...
    if (select == "1") {
        ...
        std::cout << COLOUR2 << "输入错误，请重新输入" << COLOUREND
                  << std::endl;
        system("clear");
    }
}
...
void Userui::myinformation() {
    ...
    std::cout << COLOUR3 << "账号：" << user_->getUserName() << COLOUREND
              << std::endl;
    std::cout << COLOUR3 << "ID ：" << user_->getUserId() << COLOUREND
              << std::endl;
    std::cout << COLOUR3 << "邮箱：" << user_->getUserEmail() << COLOUREND
              << std::endl;
    std::cout << COLOUR3 << "用户名：" << user_->getUserMyname() << COLOUREND
              << std::endl;
    std::cout << COLOUR3 << "密码：" << concealPwd() << COLOUREND << std::endl;
    std::cout << COLOUR3 << "输入1修改密码，输入2修改用户名，输入3退出"
              << COLOUREND << std::endl;
    std::string select1;
    std::cin >> select1;
    ...
    if (select1 == "1") {
        ...
        while (!tool::isValidInput(newmyname,
                                   "^[\u4e00-\u9fa5a-zA-Z0-9.]{1,16}$")) {
            std::cout << "输入不合法,请重新数入" << std::endl;
            std::cout << "请输入新的用户名：";
            getline(std::cin, newmyname);
...
void Userui::deleteUser() {
    ...
    if (account != user_->getUserName()) {
        std::cout << COLOUR2 << "输入错误，请稍后再试" << COLOUREND
                  << std::endl;
        return;
    }
    std::cout << "请输入密码：";
        ...
        if (Type::getUserState() == Type::USAT) {
            ...
            if (cmd == "/quit" || cmd == "\u001b") {
            ...
            } else {
                {
                    ...
                    std::string key = tool::swapsort(
                        "user:" + user_->getUserName(), "user:" + cmd, "read:");
                    chatName[key] = true;
                }
                nlohmann::json j;
            ...
            std::thread recvThread([&]() {
                while (chatting) {
                    std::queue<std::string> Messages =
                        megManager_.fetchMessages(tool::swapsort(
                            "user:" + cmd, "user:" + user_->getUserName(),
                            "read:"));
                    while (!Messages.empty()) {
                        LOG_DEBUG << "\033[1;35m" << std::this_thread::get_id()
                                  << "\033[0m";
                        std::cout << "\33[2K\r";
            ...
            while (chatting) {
                ...
                if (message == "/quit" || message == "\u001b") {
                    ...
                    cmd.clear();
                    chatCv.notify_one();
                    chatName[tool::swapsort("user:" + user_->getUserName(),
                                            "user:" + cmd, "read:")] = false;
                    break;
                    ...
                    if (message == "/file") {
                        ...
                        std::string key =
                            tool::swapsort("user:" + user_->getUserName(),
                                           "user:" + cmd, "read:");
                    ...
                    user_->preparation(j, "from",
                                       "user:" + user_->getUserName());
                    user_->send(j, conn);
                    tool::clear();
                    if (cmd != user_->getUserName()) {
                        std::cout << headerFormat("You") << message
                                  << std::endl;
                    }
                }
            }
...
bool Userui::handleCmd(std::string cmd) {
    if (cmd[0] == '/') {
        ...
        if (cmd.find(' ', pos1 + 1) == cmd.find_last_of(' ')) {
            ...
            if (pass == "/af" && (cmd.substr(end + 1) == "yes" ||
                                  (cmd.substr(end + 1) == "no"))) {
                user_->preparation(j, "account", user_->getUserName());
                user_->preparation(j, "pass", "addfriend");
                user_->preparation(j, "name",
                                   cmd.substr(start + 1, end - start - 1));
                user_->preparation(j, "result", cmd.substr(end + 1));
                user_->send(j, conn, "frie:");
                return true;
            } else if (pass == "/ag" && (cmd.substr(end + 1) == "yes" ||
                                         (cmd.substr(end + 1) == "no"))) {
                std::string owning = cmd.substr(start + 1, end - start - 1);
                user_->preparation(j, "name",
                                   owning.substr(0, owning.find_first_of(':')));
                user_->preparation(j, "pass", "addgroup");
                user_->preparation(
                    j, "group", owning.substr(owning.find_first_of(':') + 1));
                user_->preparation(j, "result", cmd.substr(end + 1));
                user_->send(j, conn, "user:");
                return true;
            } else if (pass == "/iv" && (cmd.substr(end + 1) == "yes" ||
                                         (cmd.substr(end + 1) == "no"))) {
                user_->preparation(j, "name",
                                   cmd.substr(start + 1, end - start - 1));
    ...
    while (true) {
        ...
        if (a == "\u001b" || a == "3" || a == "/quit") {
            ...
            while (true) {
                ...
                user_->preparation(j, "member" + std::to_string(count),
                                   "user:" + number);
                count++;
            }
        } else if (a == "2") {
            ...
            while (1) {
                if (Type::getUserState() == Type::URETURN) {
                    std::cout << COLOUR2 << "[WAIN] " << COLOUREND
                              << "真的要继续吗[Y/n]";
                    std::string a;
                    getline(std::cin, a);
                    if (a == "Y" || a == "y" || a == "\n") {
            ...
            std::cout << COLOUR2 << "输入错误请重新输入" << COLOUREND
                      << std::endl;
        }
    }
}
                ...
                while (1) {
                    if (Type::getUserState() == Type::URETURN) {
                        ...
                        std::thread recvThread([&]() {
                            while (chatting) {
                                std::queue<std::string> Messages =
                                    groupMegManager_.fetchMessages("grop:" +
                                                                   name);
                                while (!Messages.empty()) {
                                    std::cout << "\33[2K\r";
                                    std::cout << Messages.front() << std::endl;
                            ...
                            if (Type::getUserState() == Type::USAT) {
                                ...
                                if (message == "/quit" || message == "\u001b") {
                                    ...
                                    if (message == "/?") {
                                        ...
                                        std::cout << "/uplevel username"
                                                  << std::endl;
                                        std::cout << "/downlevel username"
                                                  << std::endl;
                                        std::cout << "/del username"
                                                  << std::endl;
                                        Type::updataUserState(Type::USAT);
                                    } else if (message == "/h") {
                                        nlohmann::json j;
                                        user_->preparation(j, "type",
                                                           "history");
                                        std::string key = "regp:" + name;
                                        user_->preparation(j, "account", key);
                                        user_->preparation(
                                            j, "name",
                                            "mygp:" + user_->getUserName());
                                        user_->send(j, conn);
                                        std::cout << "正在加载历史消息"
                                                  << std::endl;
                                    ...
                                    user_->preparation(j, "from",
                                                       "grop:" + name);
                                    std::string things =
                                        headerFormat(user_->getUserMyname()) +
                                        message;
                                    user_->preparation(j, "things", things);
                                    user_->send(j, conn, "user:");
                                    tool::clear();
                ...
                while (1) {
                    if (Type::getUserState() == Type::URETURN) {
                        ...
                        if (b == "\u001b" || b == "3" || b == "/quit") {
                            ...
                            Type::updataUserState(Type::URETURN);
                            std::cout << COLOUR2 << "输入错误" << COLOUREND
                                      << std::endl;
                        }
                    }
                }
    ...
    while (true) {
        ...
        if (input == tool::tolowerStr("LIST")) {
            ...
            if (name.substr(1, 5) == "user:") {
                std::string a = tool::swapsort("user:" + user_->getUserName(),
                                               name.substr(1), "file:");
            ...
            if (input.substr(0, 4) == tool::tolowerStr("STOR")) {
                ...
                if (ftpConn && ftpConn->connected()) {
                    ...
                    ftpConn->send(MessageSplitter::encodeMessage(
                        "info " + user_->getUserName() + name + " " +
                        user_->getUserMyname() + " " + tool::fileSize(fd) +
                        " " + input.substr(5)));
                    tool::clearInputLines(input);
                } else {
                    LOG_ERROR << "conn过期";
                ...
                while (1) {
                    if (Type::getUserState() == Type::URETURN) {
                        if (ftpConn && ftpConn->connected()) {
                            ftpConn->send(
                                MessageSplitter::encodeMessage(input));
                            tool::clearInputLines(input);
                        } else {
                            LOG_ERROR << "conn过期";
                        }
