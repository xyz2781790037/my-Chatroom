...
int main(int argc, char* argv[]) {
    ...
    auto client1Ptr =
        std::make_shared<mulib::net::TcpClient>(mainLoop.get(), addr);
    auto client2Ptr =
        std::make_shared<mulib::net::TcpClient>(mainLoop.get(), addr2);
    sendFile sendFile_(argv[1]);
    logon clientLog(client1Ptr);
    client1Ptr->setConnectionCallback(
        [&clientLog,
         &sendFile_](const mulib::net::TcpClient::TcpConnectionPtr& conn) {
            if (conn->connected()) {
                LOG_DEBUG << "连接服务器成功，可以开始交互";
                sendFile_.getmainConn(conn);
                std::thread uiThread(
                    [&clientLog]() mutable { clientLog.ui(); });
                uiThread.detach();
    ...
    client2Ptr->setConnectionCallback(
        [&sendFile_,
         &handlemeg_](const mulib::net::TcpClient::TcpConnectionPtr& conn) {
            if (conn->connected()) {
    ...
    client2Ptr->setMessageCallback(
        [&sendFile_, &megSpl1, &pool](
            const mulib::net::TcpClient::TcpConnectionPtr& conn, Buffer* buf,
            ...
            pool.enqueue(
                [messages = std::move(messages), &sendFile_, recviveTime] {
    ...
    client1Ptr->setMessageCallback(
        [&handlemeg_, &pool, &megSpl](
            const mulib::net::TcpClient::TcpConnectionPtr& conn, Buffer* buf,
            ...
            pool.enqueue([conn, recviveTime, &handlemeg_,
