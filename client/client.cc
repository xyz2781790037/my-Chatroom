...
int main()
{
    ...
    clientPtr->setMessageCallback([&handlemeg_](const mulib::net::TcpClient::TcpConnectionPtr &conn, Buffer *buf, mulib::base::Timestamp recviveTime) {  
        LOG_DEBUG << "收到消息";
        handlemeg_.recviveMeg(conn, buf, recviveTime);
