...
class handleMeg {
   public:
    void recviveMeg(const mulib::net::TcpClient::TcpConnectionPtr& conn,
...
inline void handleMeg::recviveMeg(
    ...
    if (type == Type::PRINT) {
        ...
        auto Copyconn = conn;
        auto ownPtr =
            std::make_shared<Userui>(userPtr, Copyconn, ftpConn, recviveTime);

        std::thread ownuiThread([ownPtr]() { ownPtr->ui(); });
        ownuiThread.detach();
    } else if (type == Type::MESSDATA) {
        if (jsonData["use"] == "addfriend") {
            std::string allname = jsonData["account"];
            std::string name = allname.substr(5);
            std::cout << "好友申请：" << name << "请求添加你为好友"
                      << std::endl;
        } else if (jsonData["use"] == "addgroup") {
            std::string allname = jsonData["account"];
            std::string name = allname.substr(5);
            std::cout << "加群申请：" << name << "请求进入" << jsonData["name"]
                      << std::endl;
            ...
            std::string name = allname.substr(5);
            char c = '"';
            std::cout << "邀请信息：" << name << "邀请你加入" << c
                      << gp.substr(5) << c << std::endl;
        }
    } else if (type == Type::SEE) {
        if (jsonData["see"] == "friend") {
            std::string name = jsonData["name"];
            printf("好友：%s(%s)状态：%s[%s条未读]\n", name.substr(5).c_str(),
                   jsonData["myname"].dump().c_str(),
                   jsonData["mystate"].dump().c_str(),
                   jsonData["degree"].dump().c_str());
        } else if (jsonData["see"] == "black") {
            std::string name = jsonData["name"];
            std::cout << "好友:" << name << "\033[1;35m(已拉黑)\033[0m"
                      << std::endl;
        }
        ...
        std::string key =
            tool::swapsort(jsonData["from"], jsonData["to"], "read:");
        std::string meg;
        if (chatName[key]) {
            meg = jsonData["things"];
        ...
        std::cout << "群聊：" << jsonData["name"] << "[" << jsonData["amount"]
                  << "条消息]" << std::endl;
...
inline void handleMeg::handleNoreadMessage(
    nlohmann::json& jsonData,
    const mulib::net::TcpClient::TcpConnectionPtr conn) {
    nlohmann::json j;
