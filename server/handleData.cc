...
void handleData::balckName(const TcpConnectionPtr& conn,
    ...
    if (jsonData.contains("return")) {
        if (redis.getData(jsonData["account"], jsonData["name"]) != "null") {
            ...
            conn->send(MessageSplitter::encodeMessage(
                sendMeg("已解除拉黑", Type::USAT).dump()));
        } else {
            conn->send(MessageSplitter::encodeMessage(
                sendMeg("名单上不存在", Type::USAT).dump()));
        }
        return;
    }
    if (redis.isfriend(jsonData["account"], jsonData["name"])) {
...
void handleData::deleteGroup(const TcpConnectionPtr& conn,
    ...
    if ((redis.getData("grop:" + key, jsonData["account"]) == "member" ||
         redis.getData("grop:" + key, jsonData["account"]) == "admin") &&
        !jsonData.contains("return")) {
        conn->send(MessageSplitter::encodeMessage(
            sendMeg("你正在退出群聊", Type::URETURN).dump()));
        return;
    }
    if (redis.getData("grop:" + key, jsonData["account"]) == "owner" &&
        !jsonData.contains("return")) {
        conn->send(MessageSplitter::encodeMessage(
            sendMeg("你是群主，退出后将解散群聊", Type::URETURN).dump()));
    } else if (redis.getData("grop:" + key, jsonData["account"]) == "owner" &&
               jsonData.contains("return")) {
...
void handleData::gChat(const TcpConnectionPtr& conn,
    ...
    if (redis.ismygroup(jsonData["name"], jsonData["account"])) {
        ...
        std::string key =
            tool::spellName(jsonData["name"], jsonData["account"], "grop:");
        chatStatus_[key] = true;
        conn->send(MessageSplitter::encodeMessage(
            sendMeg("开始聊天", Type::UCHAT).dump()));
    } else {
...
void handleData::handleFile(const TcpConnectionPtr& conn,
                            nlohmann::json& jsonData,
                            redisCmd& redis) {
...
void handleData::lookFile(const TcpConnectionPtr& conn,
                          nlohmann::json& jsonData,
                          redisCmd& redis) {
    if (redis.getData(jsonData["key"], jsonData["name"]) != "null") {
        LOG_INFO << "存在文件";
        conn->send(
            MessageSplitter::encodeMessage(sendMeg("", Type::URETURN).dump()));
    } else {
        conn->send(
            MessageSplitter::encodeMessage(sendMeg("", Type::USAT).dump()));
    }
}
void handleData::list(const TcpConnectionPtr& conn,
                      nlohmann::json& jsonData,
                      redisCmd& redis) {
    ...
    if (result != cpp_redis::reply()) {
        ...
        if (pan[0] == '1') {
            for (int i = 0; i + 1 < arr.size(); i += 2) {
                ...
                sum += MessageSplitter::encodeMessage(
                    sendMeg(fileName + "   " + fileSize, Type::UWAIT).dump());
            }
            conn->send(sum);
        } else if (pan[0] == '2') {
            ...
            for (int i = 0; i + 1 < arr.size(); i += 2) {
                ...
                if (rank == "owner") {
                    owner = "\033[1;33m" + myname + "(" + name.substr(5) + ")" +
                            " [群主]\033[0m";
                } else if (rank == "admin") {
                    admin.push_back("\033[1;32m" + myname + "(" +
                                    name.substr(5) + ")" + " [管理员]\033[0m");
                } else if (rank == "member") {
                    member.push_back("\033[1;38m" + myname + "(" +
                                     name.substr(5) + ")" + " [成员]\033[0m");
                }
            }
            conn->send(MessageSplitter::encodeMessage(
                sendMeg(owner, Type::UWAIT).dump()));
            for (auto ad : admin) {
                sum += MessageSplitter::encodeMessage(
                    sendMeg(ad, Type::UWAIT).dump());
            }
            for (auto mem : member) {
                sum += MessageSplitter::encodeMessage(
                    sendMeg(mem, Type::UWAIT).dump());
            }
            conn->send(sum);
        }
    }
