    ...
    while (megser.nextMessage(jsondata))
    {
        ...
        if (jsonData.contains("type")){
            ...
            if (type == Type::REGISTER){
                ...
                if(!redis.isfriend(a,b) || !redis.isfriend(b,a)){
                    std::string key = tool::spellName(a, b, "back:");
                    redis.storeReadMeg(a, b, j.dump(), key);
                    conn->send(MessageSplitter::encodeMessage(sendMeg("发出但被拒收", Type::URETURN).dump()));
                    return;
                }
                ...
                if(redis.getData(jsonData["group"],account) != "null"){
                    ...
                    conn->send(MessageSplitter::encodeMessage(sendMeg("你已被移除群聊，无法发消息", Type::UserStatus::UEXECUTE).dump()));
                }
            }
...
void handleData::verify(const TcpConnectionPtr &conn, nlohmann::json &jsonData, redisCmd &redis)
{
    ...
    else if (end == -1)
    {
        ...
        conn->send(MessageSplitter::encodeMessage(sendMeg("你已同意请求", Type::URETURN).dump()));
    }
    else if(end == -2){
        conn->send(MessageSplitter::encodeMessage(sendMeg("你拒绝请求", Type::URETURN).dump()));
    }
    else
    {
        conn->send(MessageSplitter::encodeMessage(sendMeg("信息有误！", Type::URETURN).dump()));
    }
}
